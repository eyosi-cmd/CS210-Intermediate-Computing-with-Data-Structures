...
public class Location implements Comparable<Location> {
    ...
    public boolean equals(Location that) {
        boolean lat0 = this.lat == that.lat;
        boolean lon0 = this.lon == that.lon;
        return lat0 && lon0;
    }

    // -1, 0, or 1 depending on whether the distance of this 
    // location to the origin is less than, equal to, or greater than the 
    // distance of that location to Parthenon, Athens, Greece (37.971525,
    // 23.726726).
    public int compareTo(Location that) {
        Location location = new Location("Parthenon, Athens, Greece",
        37.971525, 23.726726);
        if ((this.distanceTo(location)) 
        == (that.distanceTo(location))) {
           return 0;
        }
        else if ((this.distanceTo(location)) 
        > (that.distanceTo(location))) {
           return 1;
        }
        else 
